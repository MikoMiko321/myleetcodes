...
class Solution(object):
    def jump(self, nums):
        ...
        for i in range(len(nums) - 2, -1, -1):
            if (
                nums[i] > len(nums) - 1 - i
            ):  # If number of jumps goes outside set it to go to last
            ...
            for j in range(shadow_len):
                if (
                    i + shadow_len <= len(shadows) - 1
                    and shadows[i + j] == shadows[i + shadow_len]
                ):  # check if there is an element outside shadow and if that element is the same as the one we want to overwrite - we do nothing
                    pass
                else:
                    shadows[i + j] = i
